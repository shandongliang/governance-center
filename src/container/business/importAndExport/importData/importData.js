...
class UploadFileUpload extends React.Component {
  ...
  uploadIE = (param, values) => {
    ...
    let option = {
      url: "/tesla-gateway-console-app/importCompressedFile",
      type: "POST",
      data: { ...param, val },
      dataType: "json",
      header: {},
    ...
    $uploadfile({
      ...
      res => {
        if (res.reply.returnCode.type === "S") {
          // const { successTotalNum, failTotalNum, exceptionInfoList, messageList } = res.reply.result
          Modal.info({
            width: 800,
            // title: `成功${successTotalNum}条,异常${failTotalNum}条！`,
            title: "导入结果",
            content: (
              <pre>
                {res.reply.result}
