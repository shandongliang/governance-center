...
import { serviceUnitColumns } from "../../request/common";

const FormItem = Form.Item;
const TabPane = Tabs.TabPane;
const confirm = Modal.confirm;
export default class QueryServiceUnitData extends Component {
  ...
  async componentDidMount() {
    const { configList, serviceUnitSelectedKey, serviceUnitSelectedData } = this.props;
    await this.fetchData(configList);
    this.setState({
      selectedRowKeys: serviceUnitSelectedKey,
      selectedData: serviceUnitSelectedData
    })
  }
  ...
  fetchData = configList => {
    ...
    this.props.queryServiceUnitList(data).then(res => {
      let page = Object.assign({}, this.state.pagination, {
  ...
  onSelectServiceUnit = (record, selected) => {
    ...
    if(selected) {
      ...
      newSelectedRowKeys = selectedRowKeys.filter(item => {
        return item !== record.subModuleCode
      })
      newSelectedData = selectedData.filter(item => {
        return item.subModuleCode !== record.subModuleCode
      })
    }
    this.setState({
  ...
  onSelectAll = (selected, selectedRows, changeRows) => {
    ...
    if(selected) {
      changeRows.map(item => {
        selectedRowKeys.push(item.subModuleCode);
        selectedData.push({
          ...item,
      ...
      selectedRowKeys.map(item => {
        if(!changeRows.some(changeItem => item === changeItem.subModuleCode)) {
          newSelectedRowKeys.push(item)
        }
      })
      selectedData.map(item => {
        if(!changeRows.some(changeItem => item.subModuleCode === changeItem.subModuleCode)) {
  ...
  changeStep = step => {
    ...
    this.props.getServiceUnitData(selectedRowKeys, selectedData, step);
  }
  render() {
    ...
    const rowSelection = {
      ...
      onSelect: this.onSelectServiceUnit,
              ...
              <TabPane tab="服务单元查询" key="1">
                <div className="portlet-body">
                  ...
                  <div className="exportButton">
                    <Button.Group>
                      <Button type="primary" icon="left" onClick={() => this.changeStep("1")} >上一步</Button>
                      <Button type="primary" onClick={() => this.changeStep("3")} >下一步<Icon type="right"/></Button>
                    </Button.Group>
                  </div>
                  <Table
