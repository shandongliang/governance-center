...
export default class SubscribeIndex extends Component {
  static contextTypes = {
        ...
        // },
        {
          ...
          render: (text, record, index) => {
            return (
              <div>
                <WrapButton
                  id='EnventManager.SubRelationQuery.Edit'
                  type="primary"
  ...
  fetchData = () => {
    ...
    queryEdaSubscribeList(data).then(res => {
      let page = Object.assign({}, this.state.pagination, {
        total: res.reply.resultList.page.total
      });
      this.setState(
        {
          data: res.reply.resultList.resultList,
          pagination: page,
          loading: false
        },
        () => {
  ...
  toDetailOrEdit = (record, type) => {
    let path = {
      pathname: `/eventDriven/edaSubscribe/${type}`,
      state: {
        id: record.id
      }
    }
    goTo(path,type === "detail"?"服务订阅详情":"服务订阅编辑");
  };
  // 获取筛选数据
              ...
              <TabPane tab="订阅关系查询" key="1">
                <div className="portlet-body">
                  ...
                  <ExportBtnGroup
                    ...
                    deleteData={deleteEdaSubscribe}
                    deleteButtonId="EnventManager.SubRelationQuery.BatchDelete"
                  />
                  <Table
