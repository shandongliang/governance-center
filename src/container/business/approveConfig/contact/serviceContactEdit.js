...
class ServiceContactEdit extends Component {
    static contextTypes = {
  ...
  componentDidMount(){
    const { moduleCode, subModule, serviceCode,
      serviceVersion } = this.props.location.state
    let data = {
      moduleCode,
      subModule,
      serviceCode,
  ...
  toBack = () => {
    let path = {
			pathname: '/approve/contact/service'
		};
		goToAndClose(path, "服务联系人");
  }
  render(){
                    ...
                    <Col span={10} offset={1}>
                      <FormItem {...formItemLayout} label='交易码'>
                        {getFieldDecorator('serviceCode', 
                          {initialValue: dataDetail.serviceCode})
                          (<Input disabled />)
                        }
                      </FormItem>
                    </Col>
                    <Col span={10} offset={1}>
                      <FormItem {...formItemLayout} label='版本号'>
                        {getFieldDecorator('serviceName', 
                          {initialValue: dataDetail.serviceName})
                          (<Input disabled />)
                        }
                      </FormItem>
                    </Col>
...
export default ServiceContactEdit = Form.create()(ServiceContactEdit)