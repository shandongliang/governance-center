...
class SubModuleContactEdit extends Component {
    static contextTypes = {
  ...
  componentDidMount(){
    const { moduleCode, subModule } = this.props.location.state
    let data = {
      moduleCode,
      subModule,
      contactLevel: "1"
    }
    this.querySubModuleContact(data)
  }
  // 查询模块联系人
  querySubModuleContact = (data) => {
      ...
      this.state.list.map((item,index)=>{
        if(item.show){
          list.push({
            ...
            key: `${this.state.dataDetail.moduleCode}${this.state.dataDetail.subModuleCode}${values[`email${index}`]}`
          })
        } 
      })
      let data = {
  ...
  toBack = () => {
    let path = {
			pathname: '/approve/contact/subModule'
		};
		goToAndClose(path, "服务单元联系人");
  }
  render(){
                    ...
                    <Col span={10} offset={1}>
                      <FormItem {...formItemLayout} label='服务单元编码'>
                        {getFieldDecorator('subModuleCode', 
                          {initialValue: dataDetail.subModuleCode})
                          (<Input disabled />)
                        }
                      </FormItem>
                    </Col>
                    <Col span={10} offset={1}>
                      <FormItem {...formItemLayout} label='服务单元名称'>
                        {getFieldDecorator('subModuleName', 
                          {initialValue: dataDetail.subModuleName})
                          (<Input disabled />)
                        }
                      </FormItem>
                    </Col>
                  </Row>
                  <Button onClick={this.addContact} size="large" type="dashed" icon="plus">新增联系人</Button>
...
export default SubModuleContactEdit = Form.create()(SubModuleContactEdit)