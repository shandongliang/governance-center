...
class TopicApproveCreate extends React.Component {
  static contextTypes = {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        createEdaTopicApprove(data).then(res => {
          //返回信息
          this.setState({
            loading: false
          });
          if (res.reply.returnCode.type === "S") {
            ...
            if (this.props.returnProfile) {
              this.props.returnProfile(`${submoduleId}${values.topicName}`);
              return;
            }
            message.info("创建成功");
            this.toIndex();
...
export default Form.create()(TopicApproveCreate);
