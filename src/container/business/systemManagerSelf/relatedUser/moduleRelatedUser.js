...
export default class ModuleRelatedUser extends Component {
  ...
  componentDidMount() {
    let { modulecode } = this.props.location.state;
    this.setState({
      modulecode
    },()=>{
      this.fetchData();
  ...
  fetchData = () => {
    ...
    const { filterValue, pagination, modulecode } = this.state;
    let data = {
      sysUserModuleVo: {
        page: {
  ...
  // 创建关联模块
  createRelatedModule = () => {
    const { userId, modify, modulecode } = this.state;
    let moduleId = "";
    let data = {
      list: [{
        userId,
