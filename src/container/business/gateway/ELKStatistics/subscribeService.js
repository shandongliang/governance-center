...
export default class SubscribeService extends Component {
  static contextTypes = {
          ...
          render: (text, record) => {
            return (
              <Button
                ...
                onClick={() => this.toSubscribeServiceRecordList(record)}
              >
                {text}
              </Button>
            );
  ...
  fetchData = () => {
    ...
    queryServiceSubscribeByElk(data).then(res => {
      this.setState(
        {
          data: res.reply.subscribeList,
          loading: false
        },
        () => {
  ...
  toSubscribeServiceRecordList = record => {
    const { filterValue } = this.state;
    const { startTime, endTime } = filterValue;
    let path = {
      pathname: "/gateway/ELKStatistics/subscribeServiceRecord",
      state: {
