...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		/*
			Pandora 自定义 StencilSet，默认从后台获取
		*/
		var hasCustomStencilsetFile = !! ACTIVITI.CUSTOMCONFIG.customStencilSetFileName;
		
		var stencilUrl = KISBPM.URL.getStencilSet()

		var hasCustomStencilsetFile = !! ACTIVITI.CUSTOMCONFIG.customStencilSetFileName;

		if (hasCustomStencilsetFile) {
			stencilUrl = ACTIVITI.CUSTOMCONFIG.customConfigRoot + ACTIVITI.CUSTOMCONFIG.customStencilSetFileName
		}
		...
		if(!hasCustomStencilsetFile) {
			...
			new Ajax.Request(stencilUrl + '?version=' + Date.now(), {
	            asynchronous: false,
	            method: 'get',
    ...
    __handleStencilset: function(response){ 		
        ...
        try {
            ...
            // since there are functions in this JSON.										
						eval("this._responseText =" + response.responseText);
						var hasCustomStencilsetFile = !! ACTIVITI.CUSTOMCONFIG.customStencilSetFileName;
						this._jsonObject = hasCustomStencilsetFile ? 
							this._responseText : this._responseText.reply.result; // Tesla return format
        } 
        catch (e) {
