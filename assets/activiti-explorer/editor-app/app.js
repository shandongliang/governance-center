            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        var modelInfo = data.reply.result; // Tesla return format
                        $rootScope.editor = new ORYX.Editor(modelInfo);
                        $rootScope.modelData = angular.fromJson(modelInfo);
                        $rootScope.editorFactory.resolve();
                    }).
                    error(function (data, status, headers, config) {
                      console.log('Error loading model with id ' + modelId + ' ' + data.reply.result);
                    });
            }
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
                        ...
                        // 调整左侧 stencil set 工具栏高度、 canvas 区域高度
	                    var totalAvailable = jQuery(window).height() - offset.top - mainHeader.height() - 60; // pandora change from 21 to 60
