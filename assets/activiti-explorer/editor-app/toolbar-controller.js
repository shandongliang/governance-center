...
angular.module('activitiModeler')
    .controller('ToolbarController', ['$scope', '$http', '$modal', '$q', '$rootScope', '$translate', '$location', function ($scope, $http, $modal, $q, $rootScope, $translate, $location) {
        ...
        $scope.toolbarSecondaryButtonClicked = function(buttonIndex) {
            ...
            // 以下代码修复编辑流程图点关闭按钮不关闭nav.tabs问题，采用手动设置sessionStorage的方法
            var arr = JSON.parse(sessionStorage.getItem('$pandora_nav.tabs')).filter(function(item){
                return item.path != window.location.hash.slice(1)
            })
            sessionStorage.setItem('$pandora_nav.tabs',JSON.stringify(arr))
            window.parent.location.hash="processManager/ProcessDesigner/index"; // 跳转至 Pandora 未部署的流程模型列表页面
        };
